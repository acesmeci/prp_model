...
from prp.threshold_optimizer import optimize_threshold
...
def run_prp_trial(task_net, input_a, input_b, task_a, task_b,
    ...
    z_a = optimize_threshold(output_series, output_idxs_a, correct_response_idx=correct_a)
    ...
    z_b = optimize_threshold(output_series[onset_b:], output_idxs_b, correct_response_idx=correct_b)
...
def sweep_soa(task_net,
              ...
              soa_values,
              n_trials_per_soa=10,
              #threshold=1.0,  # Ignored; kept for legacy compatibility
              max_timesteps=50,
              tau_net=0.2,
              tau_task=0.2,
              persistence=0.0,
    ...
    for soa in soa_values:
        rt_a_list, rt_b_list = [], []
        acc_a_list, acc_b_list = [], []  # ✅ fixed unpacking error
